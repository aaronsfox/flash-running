%% This script performs a muscle-driven tracking simulation of the processed
%  Dorn et al. (2012) sprinting data. The code here is adapted from the
%  code provided by Falisse et al. (2019). Comments are made throughout for
%  the different aspects of code, but I would defer to the original
%  publication for a thorough explanation.
%
...
%%%%% NOTES: first attempt produced a small objective function (~2.2) but a
%%%%% really choppy inaccurate motion and GRFs?????
%%%%%
%%%%% Second attempt was better with only tracking kinematics and GRFs,
%%%%% while also increasing node density to 100, and doubling isometric
%%%%% force and contraction velocity.
%%%%%
%%%%% Muscle activations are still a little 'shark fin' like, which might
%%%%% suggest a finer grid density is still required.
%%%%%
...
%Set weights for problem
W.Qs = 10; % weight joint kinematics
W.GRF = 1; % weight ground reaction forces
W.GRM = 0;%1; % weight ground reaction moments
W.ID_act = 0;%10; % weight joint kinetics
W.a = 1; % weight muscle activations
dev_cm.loc = 25; % allowed contact sphere deviation from generic locations
dev_cm.rad = 50; % allowed contact sphere deviation from generic radii
N = 100;%50; % number of mesh intervals
...
%Load some variables for the polynomial approximations
...
%Set the sphere parameter indices
loci.s1.r.x = 1;
loci.s1.r.z = 2;
loci.s2.r.x = 3;
loci.s2.r.z = 4;
...
%Add path to contact model
pathContactModel = [mainPath,'\..\contact'];
...
%%%%% TODO: again, tracking guess seems to be data informed and hence
%%%%% should be fine with different data --- but may just need to confirm
...
saveName = 'trackingSim';
saveFileName = ['trackingSim_',date];
