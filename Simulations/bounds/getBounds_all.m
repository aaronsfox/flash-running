...
%%%%% TODO: these likely need to be altered to be more suitable to the
%%%%% experimental data --- for example, the translational coordinates need
%%%%% to be more suited to the running speeds being tested; the elbow
%%%%% shouldn't be locked at zero as done in Falisse et al.

% Pelvis_tx
bounds.Qs.upper(jointi.pelvis.tx) = 5;  
bounds.Qs.lower(jointi.pelvis.tx) = -5;
% Pelvis_ty
bounds.Qs.upper(jointi.pelvis.ty) = 1.25;  
bounds.Qs.lower(jointi.pelvis.ty) = 0.75;
% Pelvis_tz
bounds.Qs.upper(jointi.pelvis.tz) = 0.3;
bounds.Qs.lower(jointi.pelvis.tz) = -0.3;
% % % % Elbow
...
%%%%% TODO: for higher speeds, final time may need to be adjusted
