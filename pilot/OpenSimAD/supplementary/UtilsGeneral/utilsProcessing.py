...
def adjustMuscleWrapping(
    ...
    #Set unscaled model
    if unscaledModelFile is None:
        pathUnscaledModel = os.path.join(baseDir, 'OpenSimPipeline', 'Models',
                                         unscaledModelName + '.osim')
    else:
        pathUnscaledModel = os.path.join(pathModelFolder, unscaledModelFile)
    ...
    if scaledModelFile is None:
        ...
        pathScaledModel = os.path.join(pathModelFolder, scaledModelFile)
        
    #Set output model
    pathOutputModel = pathScaledModel[:-5]+'_adjusted.osim'
...
def generateModelWithContacts(
    ...
    #Find the adjusted model
    outputModelFileName  = glob.glob(pathModelFolder+'\\*_adjusted*')[0]    
    # outputModelFileName = (OpenSimModel + "_scaled" + suffix_MA)
    # pathOutputFiles = os.path.join(pathModelFolder, outputModelFileName)
    pathOutputModel = outputModelFileName[:-5] + "_contacts.osim"
    ...
    model = opensim.Model(outputModelFileName)   
