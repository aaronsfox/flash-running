'''
    ...
    NOTE: some edits have been made to scripts to work.
...
def generateExternalFunction(
    ...
    #Find model with adjusted contacts suffix
    pathModel = glob.glob(pathModelFolder+'\\*_adjusted_contacts*')[0]  
    ...
    pathOutputExternalFunctionFolder = os.path.join(os.getcwd(), pathModelFolder, "ExternalFunction")
    ...
    if build_externalFunction:
        pathDCAD = os.path.join(baseDir, 'supplementary', 'UtilsDynamicSimulations', 'OpenSimAD') 
    ...
    if verifyID:  
        ...
        pathGenericTemplates = os.path.join(baseDir, "supplementary", "OpenSimPipeline") 
...
def processInputsOpenSimAD(baseDir, dataFolder, session_id, trial_name,
                           ...
                           massKg = None, heightM = None,
                           unscaledModelFile = None, scaledModelFile = None, outputModelFile = None,
    ...
    # Adjust muscle wrapping (EDITS MADE - WORKS...)
    adjustMuscleWrapping(baseDir, dataFolder, session_id,
                         OpenSimModel = OpenSimModel, overwrite = overwrite,
                         unscaledModelFile = unscaledModelFile,
                         scaledModelFile = scaledModelFile,
                         outputModelFile = outputModelFile)

    # Add foot-ground contacts to musculoskeletal model (EDITS MADE - WORKS...)    
    generateModelWithContacts(dataFolder, session_id, 
                              OpenSimModel = OpenSimModel, overwrite = overwrite)
    
    # Generate external function  (EDITS MADE - WORKS...)
    ...
    if massKg is None:
        raise ValueError('A mass for the subject is required.')
    else:
        settings['mass_kg'] = massKg
    if heightM is None:
        raise ValueError('A height for the subject is required.')
    else:
        settings['height_m'] = heightM
